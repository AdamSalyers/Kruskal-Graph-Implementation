...
def union(group1, group2, vertices):
    for i in vertices:
        if i[1] == group2:
            i[1] = group1
    return vertices
...
if __name__ == '__main__':
    ...
    for (x,y) in sorted_edges:
        ...
        groupx = 0
        groupy = 0
        for i in kruskvertices:
            if i[0] == x:
                groupx = i[1]
            if i[0] == y:
                groupy = i[1]
        if groupx > 0 and groupy > 0:
            if not (groupx == groupy):
                kruskal_selected_edges.append((x,y))
                kruskvertices = union(groupx, groupy, kruskvertices)
        elif groupx > 0 or groupy > 0:
            kruskal_selected_edges.append((x,y))
            if groupx > 0:
                temp = []
                temp.append(y)
                temp.append(groupx)
                kruskvertices.append(temp)
            if groupy > 0:
                temp = []
                temp.append(x)
                temp.append(groupy)
                kruskvertices.append(temp)
        elif groupx == 0 and groupy == 0:
            #print("Adding ", x, " and ", y, " to kruskals")
            kruskal_selected_edges.append((x,y))
            temp = []
            temp.append(x)
            temp.append(groupnum)
            kruskvertices.append(temp)
            temp = []
            temp.append(y)
            temp.append(groupnum)
    ...
    #print("Length of Kruskal's is: ", len(kruskal_selected_edges))
